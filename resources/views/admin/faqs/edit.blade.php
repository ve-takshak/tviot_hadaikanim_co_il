<x-admin.layout>
	<x-admin.breadcrumb
		...
		:links="[
            ...
            ['text' => 'FAQs', 'url' => route('faqs.index')],
            ['text' => 'Edit']
		]"
        :actions="[
            ['text' => 'All FAQs', 'icon' => 'fas fa-list', 'url' => route('faqs.index'), 'permission' => 'faqs_access', 'class' => 'btn-success '],
            ['text' => 'Create FAQ', 'icon' => 'fas fa-list', 'url' => route('faqs.create'), 'permission' => 'faqs_create', 'class' => 'btn-dark '],
        ]" />

    <form method="POST" action="{{ route('faqs.update', [$faq]) }}" class="card shadow-sm">
        @csrf
        @method('PUT')
