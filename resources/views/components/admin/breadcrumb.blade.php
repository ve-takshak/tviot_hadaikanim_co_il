<div class="row align-items-center">
    ...
    @if(isset($actions) && count($actions) > 0)
	    <div class="col-sm-6">
	        <div class="dy-float  d-md-block mb-2">
	        	@if(url()->previous())
		        	<a href="{{ url()->previous() }}" class="btn none-phone  {{ (isset($back['show']) && $back['show'] === false) ? 'd-none' : '' }}" >
        				...
        				@can($action['permission'])
			            	<a 	href="{{ isset($action['url']) ? $action['url'] : 'javascript:void(0)' }}" 
								...
								@if(isset($action['attributes']))
									...
									@foreach ($action['attributes'] as $key => $attribute)
										{{ $key.'='.$attribute }}
									@endforeach
								@endif
